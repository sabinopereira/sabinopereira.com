...
guard args.count == 8 else {
    fputs("Usage: swift make-bad-girl-gospel-catalog-reel.swift <output.mp4> <audio.wav> <album> <selah> <diana> <noa> <naomi>\n", stderr)
    exit(1)
}

let outputURL = URL(fileURLWithPath: args[1])
let audioURL = URL(fileURLWithPath: args[2])
let imageURLs = args[3...7].map { URL(fileURLWithPath: $0) }
let width = 1080
let height = 1920
let fps: Int32 = 30
...
func frame(for image: CIImage, progress: CGFloat) -> CIImage {
    let backdrop = placed(image, fill: true, zoom: 1.04 + progress * 0.02)
        ...
        .cropped(to: canvas)
        .applyingFilter("CIColorControls", parameters: [kCIInputSaturationKey: 0.72, kCIInputBrightnessKey: -0.20])
    let veil = CIImage(color: CIColor(red: 0.035, green: 0.018, blue: 0.012, alpha: 0.48)).cropped(to: canvas)
...
let tempURL = outputURL.deletingLastPathComponent().appendingPathComponent("bad-girl-gospel-catalog-silent.mp4")
...
parameters.setVolume(0.82, at: .zero)
parameters.setVolumeRamp(fromStartVolume: 0.82, toEndVolume: 0, timeRange: CMTimeRange(start: CMTime(seconds: duration - 1.2, preferredTimescale: 600), duration: CMTime(seconds: 1.2, preferredTimescale: 600)))
