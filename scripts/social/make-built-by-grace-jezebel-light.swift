...
let output = root.appendingPathComponent("assets/social/posts/built-by-grace-jezebel-light/instagram")
try FileManager.default.createDirectory(at: output, withIntermediateDirectories: true)

let canvas = NSSize(width: 1080, height: 1350)
let ink = NSColor(calibratedRed: 0.975, green: 0.945, blue: 0.885, alpha: 1)
let ivory = NSColor(calibratedRed: 0.18, green: 0.13, blue: 0.085, alpha: 1)
let muted = NSColor(calibratedRed: 0.37, green: 0.29, blue: 0.21, alpha: 1)
let gold = NSColor(calibratedRed: 0.69, green: 0.39, blue: 0.12, alpha: 1)
...
func signature(_ page: Int) {
  drawText("BUILT BY GRACE · SABINO PEREIRA", NSRect(x: 80, y: 1260, width: 650, height: 30), font(18, .medium), gold, .left, 0, 3)
...
func render(_ filename: String, page: Int, content: () -> Void) {
  ...
  let glow = NSGradient(colors: [NSColor.white.withAlphaComponent(0.65), gold.withAlphaComponent(0.04)])!
...
render("01-hook.png", page: 1) {
  drawText("A BUILT BY GRACE REFLECTION", NSRect(x: 80, y: 105, width: 920, height: 45), font(24, .medium), gold, .left, 0, 5)
  rule(190)
...
let manifest = """
{
  "campaign": "Built by Grace — Jezebel advice response — light edition",
