...
DIRECTORS_DIR = BOOK_DIR / "directors-cut"
SOURCE_DIR = DIRECTORS_DIR
EBOOK_DIR = DIRECTORS_DIR / "premium-edition/ebook"
PAPERBACK_DIR = DIRECTORS_DIR / "premium-edition/paperback"
PREVIEW_DIR = DIRECTORS_DIR / "premium-edition/previews"
...
DIGITAL_PDF = DIRECTORS_DIR / "premium-edition/digital/quiet-power-premium-digital-edition.pdf"
EXTRACTED_TEXT = DIRECTORS_DIR / "quiet-power-directors-cut-draft.md"
OFFICIAL_COVER = DIRECTORS_DIR / "official/quiet-power-official-cover-front.png"
METADATA_JSON = DIRECTORS_DIR / "premium-edition/kdp-metadata.json"

TITLE = "Quiet Power"
SUBTITLE = "Calm. Focused. Unshaken. Design."
AUTHOR = "Sabino Pereira"
BOOK_ID = "urn:uuid:" + str(uuid.uuid5(uuid.NAMESPACE_URL, "https://sabinopereira.com/quiet-power-directors-cut"))
TRIM_W = 6 * inch
TRIM_H = 9 * inch
BLEED = 0.125 * inch
WHITE_PAPER_SPINE_PER_PAGE = 0.002252 * inch
NAVY = colors.HexColor("#102F58")
DEEP_NAVY = colors.HexColor("#071C35")
GOLD = colors.HexColor("#C79B53")
...
def parse_sections() -> list[Section]:
    ...
    def flush() -> None:
        nonlocal title, subtitle, lines
        if not title:
            return
        if title.startswith("PART "):
            kind = "part"
        elif title == "Final Manifesto":
            kind = "manifesto"
        elif title in {"The Quiet Power Standard", "The 30-Day Quiet Power Installation"}:
            kind = "practice"
        elif title.startswith("Chapter "):
            kind = "chapter"
        else:
            kind = "intro"
        sections.append(Section(kind, title, subtitle, lines))
        title, subtitle, lines = "", "", []

    for raw in EXTRACTED_TEXT.read_text(encoding="utf-8").splitlines():
        line = raw.strip()
        if line.startswith("# "):
            flush()
            title = line[2:].strip()
        elif line.startswith("## ") and title and not subtitle and not lines:
...
def create_cover(path: Path) -> None:
    ...
    w, h = 1600, 2560
    source = Image.open(OFFICIAL_COVER).convert("RGB")
    scale = max(w / source.width, h / source.height)
    resized = source.resize((round(source.width * scale), round(source.height * scale)), Image.Resampling.LANCZOS)
    left = (resized.width - w) // 2
    top = (resized.height - h) // 2
    im = resized.crop((left, top, left + w, top + h))
    im.save(path, "JPEG", quality=96, optimize=True)
...
class DigitalCoverPage(Flowable):
    ...
    def draw(self):
        self.canv.drawImage(self.path, 0, 0, TRIM_W, TRIM_H, preserveAspectRatio=False, mask="auto")


def draw_digital_cover_background(canv: canvas.Canvas, doc: BaseDocTemplate) -> None:
    canv.saveState()
    canv.setFillColor(DEEP_NAVY)
    canv.rect(0, 0, TRIM_W, TRIM_H, fill=1, stroke=0)
    canv.restoreState()


def draw_digital_page(canv: canvas.Canvas, doc: BaseDocTemplate) -> None:
    canv.saveState()
    canv.setFillColor(CREAM)
    canv.rect(0, 0, TRIM_W, TRIM_H, fill=1, stroke=0)
    canv.setStrokeColor(GOLD)
    canv.setLineWidth(0.55)
    canv.line(0.72 * inch, TRIM_H - 0.52 * inch, TRIM_W - 0.72 * inch, TRIM_H - 0.52 * inch)
    canv.setFont("Georgia", 7.4)
    canv.setFillColor(MUTED)
    canv.drawString(0.72 * inch, 0.42 * inch, TITLE)
    canv.drawRightString(TRIM_W - 0.72 * inch, 0.42 * inch, str(doc.page))
    canv.restoreState()


def digital_rule() -> Flowable:
    class Rule(Flowable):
        def wrap(self, avail_width, avail_height):
            self.width = avail_width
            return avail_width, 18
...
def build_digital_pdf(sections: list[Section]) -> None:
    DIGITAL_PDF.parent.mkdir(parents=True, exist_ok=True)
    base = getSampleStyleSheet()
    s = {
        "title": ParagraphStyle("DigitalTitle", parent=base["Title"], fontName="Georgia-Bold", fontSize=31, leading=36, textColor=NAVY, alignment=TA_CENTER, spaceAfter=14),
        "subtitle": ParagraphStyle("DigitalSubtitle", parent=base["Normal"], fontName="Georgia-Italic", fontSize=13, leading=19, textColor=GOLD, alignment=TA_CENTER, spaceAfter=10),
        "author": ParagraphStyle("DigitalAuthor", parent=base["Normal"], fontName="Georgia-Bold", fontSize=11.5, leading=16, textColor=NAVY, alignment=TA_CENTER, spaceBefore=18),
        "fronthead": ParagraphStyle("DigitalFrontHead", parent=base["Heading1"], fontName="Georgia-Bold", fontSize=22, leading=27, textColor=NAVY, alignment=TA_CENTER, spaceAfter=15),
        "front": ParagraphStyle("DigitalFront", parent=base["Normal"], fontName="Georgia", fontSize=10.5, leading=16.5, textColor=INK, spaceAfter=9),
        "small": ParagraphStyle("DigitalSmall", parent=base["Normal"], fontName="Georgia", fontSize=8.8, leading=13.5, textColor=MUTED, spaceAfter=8),
        "toc": ParagraphStyle("DigitalToc", parent=base["Normal"], fontName="Georgia", fontSize=9.6, leading=14.5, textColor=NAVY, spaceAfter=2),
        "part": ParagraphStyle("DigitalPart", parent=base["Heading1"], fontName="Georgia-Bold", fontSize=25, leading=31, textColor=NAVY, alignment=TA_CENTER, spaceAfter=9),
        "partsub": ParagraphStyle("DigitalPartSub", parent=base["Normal"], fontName="Georgia-Italic", fontSize=13, leading=18, textColor=GOLD, alignment=TA_CENTER, spaceAfter=20),
        "chapter": ParagraphStyle("DigitalChapter", parent=base["Heading1"], fontName="Georgia-Bold", fontSize=21, leading=26, textColor=NAVY, alignment=TA_CENTER, spaceAfter=7),
        "chaptersub": ParagraphStyle("DigitalChapterSub", parent=base["Normal"], fontName="Georgia-Italic", fontSize=11.5, leading=17, textColor=GOLD, alignment=TA_CENTER, spaceAfter=16),
        "body": ParagraphStyle("DigitalBody", parent=base["Normal"], fontName="Georgia", fontSize=10.4, leading=15.8, textColor=INK, spaceAfter=4.2),
        "manifestobody": ParagraphStyle("DigitalManifestoBody", parent=base["Normal"], fontName="Georgia", fontSize=10.1, leading=15.1, textColor=INK, spaceAfter=3.4),
        "bullet": ParagraphStyle("DigitalBullet", parent=base["Normal"], fontName="Georgia", fontSize=10.2, leading=15.3, textColor=INK, leftIndent=18, firstLineIndent=-10, spaceAfter=3),
        "subhead": ParagraphStyle("DigitalSubhead", parent=base["Heading2"], fontName="Georgia-Bold", fontSize=11.8, leading=16, textColor=NAVY, spaceBefore=9, spaceAfter=4),
        "practice": ParagraphStyle("DigitalPractice", parent=base["Heading2"], fontName="Georgia-Bold", fontSize=14.5, leading=19, textColor=GOLD, spaceBefore=13, spaceAfter=7),
    }
    doc = BaseDocTemplate(str(DIGITAL_PDF), pagesize=(TRIM_W, TRIM_H), leftMargin=0, rightMargin=0, topMargin=0, bottomMargin=0, title=f"{TITLE} - Premium Digital Edition", author=AUTHOR, subject=SUBTITLE)
    cover_frame = Frame(0, 0, TRIM_W, TRIM_H, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0, id="digital-cover")
    body_frame = Frame(0.72 * inch, 0.65 * inch, TRIM_W - 1.44 * inch, TRIM_H - 1.25 * inch, leftPadding=0, rightPadding=0, topPadding=0.22 * inch, bottomPadding=0.1 * inch, id="digital-body")
    doc.addPageTemplates([
        PageTemplate(id="DigitalCover", frames=[cover_frame], onPage=draw_digital_cover_background, autoNextPageTemplate="DigitalBody"),
        PageTemplate(id="DigitalBody", frames=[body_frame], onPage=draw_digital_page),
    ])
    story: list = [DigitalCoverPage(COVER_JPG), PageBreak()]
    story.extend([
        Spacer(1, 0.92 * inch),
        paragraph(TITLE.upper(), s["title"]), digital_rule(), Spacer(1, 12),
        paragraph(SUBTITLE, s["subtitle"]),
        paragraph("PREMIUM DIGITAL EDITION", s["author"]),
        paragraph(AUTHOR, s["author"]),
        PageBreak(), Spacer(1, 0.48 * inch),
        paragraph("Copyright", s["fronthead"]), digital_rule(), Spacer(1, 16),
        paragraph("Copyright © 2026 Sabino Pereira. All rights reserved.", s["small"]),
        paragraph("No part of this publication may be reproduced, stored, distributed, or transmitted in any form or by any means without prior written permission from the copyright holder, except for brief quotations used in reviews.", s["small"]),
        paragraph("This book is for informational and reflective purposes only. It is not a substitute for medical, psychological, legal, or financial advice.", s["small"]),
        paragraph("Premium digital edition, 2026", s["small"]),
        PageBreak(), Spacer(1, 0.34 * inch),
        paragraph("Contents", s["fronthead"]), digital_rule(), Spacer(1, 10),
    ])
    for section in sections:
        story.append(paragraph(section.title, s["toc"]))
    story.append(PageBreak())
    for section in sections:
        if section.kind == "part":
            story.extend([Spacer(1, 2.45 * inch), paragraph(section.title, s["part"]), digital_rule(), paragraph(section.subtitle, s["partsub"]), PageBreak()])
            story.extend([Spacer(1, 0.42 * inch), paragraph(section.title.replace(":", " -"), s["chapter"]), digital_rule(), Spacer(1, 10)])
        else:
            story.extend([Spacer(1, 0.48 * inch), paragraph(section.title, s["chapter"])])
            if section.subtitle:
                story.append(paragraph(section.subtitle, s["chaptersub"]))
            story.extend([digital_rule(), Spacer(1, 9)])
        for line in section.lines:
            clean = line.removeprefix("## ")
            if line.startswith("## Field Practice"):
                story.append(paragraph(clean, s["practice"]))
            elif line.startswith("## "):
                story.append(paragraph(clean, s["subhead"]))
            elif line.startswith("•"):
                story.append(paragraph(line, s["bullet"]))
            elif re.fullmatch(r"\d+\..+", line):
                story.append(paragraph(line, s["subhead"]))
            elif len(line) < 44 and not line.endswith((".", "?", "!", ":", ";")) and not line.startswith("“"):
                story.append(paragraph(line, s["subhead"]))
            else:
                story.append(paragraph(line, s["manifestobody"] if section.kind == "manifesto" else s["body"]))
        story.append(PageBreak())
    doc.build(story)
...
def build_interior(sections: list[Section]) -> int:
    ...
    story: list = [
        ...
        paragraph("PREMIUM EDITION", styles["BookAuthor"]),
        paragraph(AUTHOR, styles["BookAuthor"]),
        PageBreak(),
        Spacer(1, 2.0 * inch),
        paragraph("Premium Director's Cut", styles["QPSectionHead"]),
    ...
    for section in sections:
        ...
        if section.kind == "practice":
            ...
            for line in section.lines:
                if re.fullmatch(r"\d+\..+", line) or line == "Closing Standard" or line.startswith("## "):
                    flush_intro()
                    flush_block()
        ...
        for line in section.lines:
            if line.startswith("## "):
                story.append(paragraph(line.removeprefix("## "), styles["QPPracticeTitle"]))
            elif line.startswith("•"):
...
''')
    write(build_dir / "OEBPS/styles.css", """body{font-family:Georgia,serif;line-height:1.62;margin:0 7%;color:#1c2730;background:#f7f2e7}.section{break-before:page;page-break-before:always}h1{color:#102f58;font-size:1.5em;line-height:1.24;text-align:center;margin:2.1em 0 .7em;padding-bottom:.55em;border-bottom:1px solid #c79b53}.subtitle{color:#9b7039;font-style:italic;text-align:center;margin-bottom:1.7em}.part-opener{break-before:page;page-break-before:always;text-align:center;margin-top:38%}.part-opener h1{text-transform:uppercase;letter-spacing:.06em}.part-intro{break-before:page;page-break-before:always}.part-intro h1{font-size:1.2em}h2{color:#102f58;font-size:1.12em;line-height:1.35;margin:1.45em 0 .5em}.practice h2{color:#9b7039}.cover{text-align:center;margin:0;background:#071c35}.cover img{max-width:100%;height:auto}p{margin:0 0 .78em}nav a{color:#102f58;text-decoration:none}nav ol{line-height:1.8}""")
    ...
    for i, section in enumerate(sections, 1):
        ...
        for line in section.lines:
            if line.startswith("## "):
                parts.append(f"<h2>{html.escape(line.removeprefix('## '))}</h2>")
            elif re.fullmatch(r"\d+\..+", line):
...
<metadata xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:identifier id="bookid">{BOOK_ID}</dc:identifier><dc:title>{html.escape(TITLE)}</dc:title><dc:creator>{AUTHOR}</dc:creator><dc:language>en</dc:language><dc:publisher>{AUTHOR}</dc:publisher><meta name="cover" content="cover-image"/><meta property="dcterms:modified">2026-07-20T00:00:00Z</meta></metadata>
...
def write_metadata(specs: dict[str, float]) -> None:
    METADATA_JSON.write_text(json.dumps({
        ...
        "subtitle": SUBTITLE,
        "author": AUTHOR,
        "edition": "Premium Director's Cut",
        "kdp_setup": {
            "trim_size": "6 x 9 in",
